...
export default function DashboardPage() {
  ...
  const { lastScores, badges, isLoading, error, refreshScores } = useScore()
  const [profile, setProfile] = useState<UserProfile | null>(null)
  const [showHeartBurst, setShowHeartBurst] = useState(false)

  // Auto-login once per wallet connection (prevents loops / double prompts)
  const { isAuthenticated, login, token } = useAuth()
  const triedLoginRef = useRef<string | null>(null)
  useEffect(() => {
    if (!isConnected || !address) return
    // only attempt once for the current address
    if (!isAuthenticated && triedLoginRef.current !== address) {
      triedLoginRef.current = address
      login().catch(() => {
        // swallow: UI
      })
    }
  ...
  // Load profile ONLY after confirmed on-chain Score Card (prevents stale/unauthorized views)
  useEffect(() => {
    ...
    if (isConnected && address && token && lastScores && lastScores.total_score > 0) {
      loadProfile()
    } else {
      setProfile(null)
    }
  }, [isConnected, address, lastScores, token])

  // Live profile updates (settings page → dashboard)
  useEffect(() => {
    const onProfileUpdated = () => {
      if (isConnected && address && token && lastScores && lastScores.total_score > 0) {
        ProfileManager.loadProfile(address, token).then(setProfile).catch(() => {
          setProfile({
            username: '',
            avatar: '/default-avatar.svg',
            useBasenameProfile: false,
    ...
    const onProfileDataUpdated = (event: Event) => {
      const e = event as CustomEvent
      if ((e as any).detail?.address === address) {
        setProfile((e as any).detail.profile)
      }
    }
  ...
  return (
    <div className="min-h-screen text-white relative overflow-hidden">
      ...
      {/* Error banner (keep user on page) */}
      {error && (
        <motion.div
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          {/* Badges */}
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="lg:col-span-2 lg:col-start-2">
            {lastScores && lastScores.total_score > 0 ? (
                    ...
                    >
                      <div className="text-center mb-3">
                        <div className="text-2xl lg:text-3xl mb-2 relative inline-block transition-transform duration-300 group-hover:scale-110">
                          ...
                          {!badge.earned && <span className="absolute -top-2 -right-3 text-xs lg:text-sm opacity-80">🔒</span>}
                        </div>
                      </div>

                      <div className="text-center">
                        <h3 className={`text-xs lg:text-sm font-semibold mb-1 ${badge.earned ? 'text-white' : 'text-gray-400'}`}>
                          {badge.name}
                        </h3>
                        <p className={`text-xs leading-relaxed ${badge.earned ? 'text-gray-300' : 'text-gray-500'}`}>{badge.description}</p>
                        {badge.earned && (
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm leading-relaxed text-gray-300">
                This dashboard summarizes your latest score, badge previews, and progress. Use the sidebar to jump to Score
                Card for new checks or to the Badge page. Settings are available from the header.
              </p>
            </div>
          </div>
        </motion.div>
        ...
        {showHeartBurst && (
          <>
            <motion.div
              ...
              animate={{ opacity: [0, 1, 0], scale: [0, 1.2, 0.8], x: [0, 80, 120], y: [0, -60, -100], rotate: [0, 180, 360] }}
              transition={{ duration: 1.8, ease: 'easeOut' }}
              className="fixed z-50 pointer-events-none"
              style={{ left: 'calc(50% - 700px)', top: 'calc(50% - 250px)' }}
            >
            ...
            <motion.div
              ...
              animate={{ opacity: [0, 1, 0], scale: [0, 1.5, 1], y: [0, -20, -40] }}
