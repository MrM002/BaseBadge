...
export default function FaultyTerminal({
  ...
  const handleMouseEnter = useCallback(() => {
    // Reset mouse position when entering container
    mouseRef.current = { x: 0.5, y: 0.5 };
    smoothMouseRef.current = { x: 0.5, y: 0.5 };
  }, []);
  ...
  useEffect(() => {
    ...
    const update = (t: number) => {
      ...
      // Enhanced mouse interaction with better smoothing
      if (mouseReact && program.uniforms.uMouse) {
        const dampingFactor = 0.1; // Increased for more responsive mouse tracking
        ...
        // Update the uniform with smooth mouse position
        const mouseUniform = program.uniforms.uMouse.value;
        mouseUniform[0] = smoothMouse.x;
        mouseUniform[1] = smoothMouse.y;
        
        // Force uniform update
        program.uniforms.uMouse.needsUpdate = true;
      }
